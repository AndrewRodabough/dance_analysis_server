...
model = dict(
    type='OCSORT',  # CHANGED: Top level is now the Tracker
    
    # 1. THE TRACKER CONFIGURATION
    tracker=dict(
        type='OCSORTTracker',
    ...
    # 2. THE DETECTOR CONFIGURATION (Your RTMDet Model goes here)
    detector=dict(
        type='RTMDet',
        init_cfg=dict(
            type='Pretrained',
...
# 3. PIPELINES (Kept exactly as you provided)
train_pipeline = [
...
# 4. LOAD WEIGHTS (Crucial for Inference)
