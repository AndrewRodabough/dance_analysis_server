...
def _get_r2_client():
    """Lazy R2 client (for routine video uploads)."""
    import os
    return boto3.client(
        "s3",
        endpoint_url=os.getenv("R2_ENDPOINT", "https://example.r2.cloudflarestorage.com"),
...
def create_presigned_put_url(
    ...
    import os
    bucket = os.getenv("R2_BUCKET", "dance-videos-r2")
...
def create_presigned_get_url(
    ...
    """Generate a presigned GET URL for downloading from MinIO."""
...
def head_object(storage_key: str) -> bool:
    """Check if an object exists in MinIO. Returns True if it does."""
    client = _get_minio_client()
    try:
