...
# Anatomical constraints disabled - was causing joints to jump
# from app.analysis.pose_estimation.anatomical_constraints import apply_constraints_to_sequence

# Set to True to enable experimental anatomical constraint filter
...
def pose_estimation(filepath_in: str, apply_smoothing: bool):
    ...
    # Store original keypoints before any corrections (for comparison visualization)
    all_keypoints_3d_before = [kp.copy() for kp in all_keypoints_3d]
    
    # Anatomical constraints - uses reflection across limb axis
    if ENABLE_ANATOMICAL_CONSTRAINTS:
        from app.analysis.pose_estimation.anatomical_constraints import apply_constraints_to_sequence
        print("Applying anatomical constraints to fix impossible joint angles...")
    ...
    # Return before/after data for comparison visualization
    return all_keypoints_2d, all_keypoints_3d, all_keypoints_3d_before, all_scores
